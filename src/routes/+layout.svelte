...
{#snippet Topbar()}
	...
	>
		...
		<div class="flex gap-4">
				...
				<button
					...
					onpointerup={() => {
						skstate.updateSettings({
							displayMode: skstate.displayMode === 'condensed' ? 'expanded' : 'condensed'
						});
					}}
				>
					{@render Icon(skstate.displayMode === 'condensed' ? 'arrows-out' : 'arrows-in')}
				</button>
			{/if}
			<a
			...
			>
				{@render Icon(skstate.darkMode ? 'sun' : 'moon')}
			</button>
		</div>
	</div>
{/snippet}

{#snippet Icon(icon: 'sun' | 'moon' | 'arrows-in' | 'arrows-out')}
	<svg
		xmlns="http://www.w3.org/2000/svg"
		fill="none"
		viewBox="0 0 24 24"
	...
	>
		...
		{:else if icon === 'moon'}
			...
			/>
		{:else if icon === 'arrows-in'}
			<path
				stroke-linecap="round"
				stroke-linejoin="round"
				d="M9 9V4.5M9 9H4.5M9 9 3.75 3.75M9 15v4.5M9 15H4.5M9 15l-5.25 5.25M15 9h4.5M15 9V4.5M15 9l5.25-5.25M15 15h4.5M15 15v4.5m0-4.5 5.25 5.25"
			/>
		{:else if icon === 'arrows-out'}
			<path
				stroke-linecap="round"
				stroke-linejoin="round"
