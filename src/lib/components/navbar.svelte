...
<div
	class="fixed bottom-0 z-10 flex h-16 w-screen flex-row items-center justify-center gap-6 bg-donkey-200 p-2 dark:bg-donkey-900 md:h-screen md:w-20 md:flex-col md:justify-start md:gap-8"
>
	<button
		class="mb-4 hidden rounded-xl bg-donkey-300 drop-shadow-lg hover:bg-donkey-400 dark:bg-donkey-900 dark:drop-shadow-none hover:dark:bg-donkey-800 md:block"
		onpointerup={() => {
			vibrate();
			goto('/');
		}}
...
{#snippet IndexButton(buttonName: 'moments' | 'characters' | 'relationships' | 'locations')}
	...
	>
		<svg
			...
			viewBox="0 0 24 24"
			stroke-width={currIndex === buttonName ? 0 : 1.5}
			stroke="currentColor"
			fill={buttonName === currIndex ? 'currentColor' : 'none'}
			class="size-8 {currIndex === buttonName
